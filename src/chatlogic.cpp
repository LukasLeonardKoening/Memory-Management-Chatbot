            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // Comment: Replaced raw pointer by a REFERENCE of the unique pointer. Usage of pure unique pointers would not work because
                        //          the function find_if would try to make copies of the unique pointers (forbidden!).
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](auto& node) { return node->GetID() == id; });

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            // Usage of make_unique because copies of unique pointers are not allowed per definition
                            // and emplace back tries to copy an instance in order to add it at the back of the vector
